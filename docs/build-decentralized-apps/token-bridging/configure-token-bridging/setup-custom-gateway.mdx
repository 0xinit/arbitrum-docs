...
This step will depend on your setup. In this case, since our simplified gateway supports only one token, we'll deploy it on both the parent and child chains to enable calling the `setTokenBridgeInformation` method on both gateways afterwards.

We won't go through the process of deploying custom tokens in this how-to, but you can see a detailed explanation of the steps to take on the [generic-custom gateway setup page](/build-decentralized-apps/token-bridging/configure-token-bridging/setup-generic-custom-gateway.mdx).
