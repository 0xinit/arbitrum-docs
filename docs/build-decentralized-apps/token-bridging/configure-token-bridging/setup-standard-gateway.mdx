...
2. **Automatic child chain deployment**: On the first deposit, a [`StandardArbERC20`](https://github.com/OffchainLabs/token-bridge-contracts/blob/main/contracts/tokenbridge/arbitrum/StandardArbERC20.sol) contract is deployed on the child chain
3. **Escrow model**: Parent chain tokens are escrowed in the gateway; child chain tokens are minted and burned
...
## Step 1: Deploy your token (or use an existing)
...
## Step 3: Trigger child chain's token deployment
...
For these cases, use a [generic-custom gateway](/build-decentralized-apps/token-bridging/configure-token-bridging/setup-generic-custom-gateway.mdx) or a [custom gateway](/build-decentralized-apps/token-bridging/configure-token-bridging/setup-custom-gateway.mdx).
...
The gateway controls the automatically deployed child chain token—you cannot modify or upgrade it. For control over the child chain token, use the generic-custom gateway.
