...
The security of Nitro chains depends on the assumption that when one validator creates an assertion, other validators will verify it and respond with a correct assertion or a challenge if it's incorrect. This assumption requires that the other validators have the time and resources to check each assertion and issue a challenge quickly. The Arbitrum protocol accounts for this when setting assertion deadlines.
