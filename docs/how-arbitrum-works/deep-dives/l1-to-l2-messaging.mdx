...
Retryable tickets power Arbitrum's canonical token bridge. For the full token bridge architecture, including how `ETH` and `ERC-20` tokens are bridged between layers, see the [Token bridging overview](/how-arbitrum-works/deep-dives/token-bridging.mdx).
