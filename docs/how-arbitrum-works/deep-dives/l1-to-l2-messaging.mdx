...
1. **General cross-chain messaging**: Allows parent-chain EOA or contract to send messages or transactions to a child chain. This functionality is critical for bridging assets (other than the chain's native token) and performing cross-chain operations.
...
Please refer to the [Address aliasing](#address-aliasing) discussion for more background. This mechanism ensures that a parent chain contract can't impersonate a child chain address unless it provides a valid signature as an EOA.
...
- A call to `inbox.createRetryableTicket` function that puts the message in the child chain inboxwhich can be re-executed for a fixed amount of time if it reverts.

3. Check the user's deposit

- A logic check to see if the user has enough funds to create a ticket. A process that checks if the `msg.value` provided by the user is greater than or equal to `maxSubmissionCost + l2CallValue + gasLimit * maxFeePerGas`.
...
- if the `maxFeePerGas` (provided by the user in the ticket submission process) is greater than (or equal to) the `l2BaseFee`. If both conditions are met, an attempt to execute the ticket on the child chain triggers (i.e., auto-redeem using the supplied gas, as if the `[ArbyRetryableTx]` \* redeem method had been called). Depending on how much gas the sender has provided in Step 1, the ticket’s redemption can either (1) immediately succeed or (2) fail. We explain both situations below:
...
During ticket redemption, attempts to cancel the same ticket or to schedule another `redeem` of the same ticket will be reverted. In this manner, retryable tickets are not self-modifying.
