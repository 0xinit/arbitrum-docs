...
Yes, we have documented fast deposits in [how to configure delayed inbox finality](/launch-arbitrum-chain/how-tos/arbitrum-chain-finality.mdx).
...
It's possible to run it in the same container. That is the default way, and then Nitro uses a loopback connection. You can read more about this in [running a split-validator](/run-arbitrum-node/more-types/04-run-split-validator-node.mdx).
