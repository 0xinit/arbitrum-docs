...
Yes, we have documented fast deposits in [how to configure delayed inbox finality](/launch-arbitrum-chain/02-configure-your-chain/common/validation-and-security/arbitrum-chain-finality.mdx).
...
It's possible to run it in the same container. That is the default way, and then Nitro uses a loopback connection. You can read more about this in [running a split-validator](/launch-arbitrum-chain/07-arbitrum-node-runners/04-run-split-validator-node.mdx).
