...
The SDK uses the `ConvertInt` trait internally to enable conversions between Alloy’s `Uint<BITS, LIMBS>` types and Rust's native integer types, such as `u8`, `u16`, `u32`, `u64`, and `u128`.
