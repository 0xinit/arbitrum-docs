...
Even with caches sized correctly and `GOMEMLIMIT` tuned, transient workload spikes or unexpected memory pressure can push a node toward OOM. Nitro provides two built-in memory protection mechanisms that use Linux cgroups to monitor actual container memory usage at runtime and take protective action before the kernel's OOM killer intervenes.

Both mechanisms work by reading the container's cgroup memory files (v1 or v2), computing the effective memory usage excluding page cache (which the kernel can reclaim), and comparing it against the container's memory limit minus a configurable free-memory threshold:
...
By subtracting active and inactive file cache from usage, the check avoids false positives caused by the kernel's page cache, which can be reclaimed under pressure and doesn't represent actual memory consumption.

:::note
Both features require Linux cgroups (v1 or v2) and only work inside containers or cgroup-limited environments. On bare-metal hosts without cgroup limits, the features will be unavailable.
:::

### RPC throttling

This setting protects the node by rejecting incoming RPC requests with HTTP 429 (Too Many Requests) when free memory drops below the configured threshold. It acts as a back-pressure mechanism, preventing new RPC work from pushing the node over its memory limit.
...
| ----------------------------- | --------- | ----------------------------------------------------------------------------- |
| `arb/rpc/limitcheck/duration` | Histogram | Time spent checking the memory limit per request (nanoseconds).               |
| `arb/rpc/limitcheck/success`  | Counter   | Number of RPC requests allowed through.                                       |
| `arb/rpc/limitcheck/failure`  | Counter   | Number of RPC requests rejected due to memory pressure.                       |
| `arb/memory/limit`            | Gauge     | Current effective memory limit (cgroup limit minus free threshold), in bytes. |
| `arb/memory/usage`            | Gauge     | Current effective memory usage (cgroup usage minus page cache), in bytes.     |

**Log lines:**

| Level | Message                                                                                                  | When                                                          |
| ----- | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| Error | `No method for determining memory usage and limits was discovered, disabled memory limit RPC throttling` | Cgroups v1/v2 not available; throttling silently disabled.    |
| Info  | `Cgroups v1 detected, enabling memory limit RPC throttling`                                              | Cgroups v1 detected at startup.                               |
| Info  | `Cgroups v2 detected, enabling memory limit RPC throttling`                                              | Cgroups v2 detected at startup.                               |
| Error | `Error checking memory limit`                                                                            | An error occurred reading cgroup files during an RPC request. |

### Block validator throttling

This setting protects the node by pausing block validation when free memory drops below the configured threshold. Block validation is memory-intensive — it involves recording and replaying blocks through the WASM validator — so pausing it under memory pressure prevents the validation workload from triggering an OOM kill.
...
Unlike the RPC throttling setting, this is **enabled by default** with a 1 GB threshold. When free memory drops below the threshold, the block validator pauses both recording new blocks and sending pending validations. Validation resumes automatically once memory usage drops back below the threshold.
...
| Metric                                | Type  | Description                                                                   |
| ------------------------------------- | ----- | ----------------------------------------------------------------------------- |
| `arb/validator/memory/limit_exceeded` | Gauge | `1` when the block validator is paused due to memory pressure, `0` otherwise. |

**Log lines:**

| Level | Message                                                                                         | When                                                                           |
| ----- | ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| Error | `sendNextRecordRequests: aborting due to running low on memory`                                 | Block recording paused because free memory is below threshold.                 |
| Error | `sendValidations: aborting due to running low on memory`                                        | Sending validations paused because free memory is below threshold.             |
| Error | `error checking if free-memory limit exceeded using MemoryFreeLimitChecker`                     | An error occurred reading cgroup files during a validation check.              |
| Warn  | `Cgroups V1 or V2 is unsupported, memory-free-limit feature inside block-validator is disabled` | Cgroups not available and default setting was used; feature silently disabled. |

:::tip
Alert on `arb/validator/memory/limit_exceeded == 1` to know when your validator nodes are under memory pressure and validation is paused. Sustained pauses may indicate that the node needs more memory or that cache sizes should be reduced.
:::

### Choosing between the two settings

The two settings protect different subsystems and can be used independently or together:

| Setting                                  | Protects         | Action                              | Default  |
| ---------------------------------------- | ---------------- | ----------------------------------- | -------- |
| `node.resource-mgmt.mem-free-limit`      | RPC serving      | Rejects new RPC requests (HTTP 429) | Disabled |
| `node.block-validator.memory-free-limit` | Block validation | Pauses recording and validation     | 1 GB     |

For **RPC nodes** that don't run validation, `node.resource-mgmt.mem-free-limit` is the relevant setting. For **validator nodes**, the block validator setting is already active by default and provides protection against validation-driven OOM.
...
5. **Enable RPC throttling on public-facing nodes**: Set `node.resource-mgmt.mem-free-limit` (e.g., `1GB`) on nodes that serve external RPC traffic. This prevents request surges from pushing the node into OOM. Monitor `arb/rpc/limitcheck/failure` to track how often throttling activates.
