...
### RPC throttling (`node.resource-mgmt.mem-free-limit`)
...
### Block validator throttling (`node.block-validator.memory-free-limit`)
...
For **RPC nodes** that don't run validation, `node.resource-mgmt.mem-free-limit` is the relevant setting. For **validator nodes**, the block validator setting is already active by default and provides protection against validation-driven OOM. On nodes that serve both RPC and validation, enabling both provides defense-in-depth.
