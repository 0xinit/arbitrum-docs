...
Nitro emits metrics under the `arb/rpc/limitcheck/` and `arb/memory/` namespaces to track accepted/rejected requests and current memory usage. It also logs at startup whether cgroup-based throttling was successfully enabled, and logs errors if memory checks fail at runtime.
...
The two settings protect different subsystems and can be used independently or together. For RPC nodes that don't run validation, `node.resource-mgmt.mem-free-limit` is the relevant setting. For validator nodes, the block validator setting is already active by default and provides protection against validation-driven OOM.
