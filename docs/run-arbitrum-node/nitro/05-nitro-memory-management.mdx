...
**Stylus WASM cache** stores compiled WebAssembly modules for Stylus smart contracts. It is allocated via Rust (invoked through CGO FFI) and bounded by `stylus-lru-cache-capacity`.
...
| `stylus-lru-cache-capacity` | 256 MB  | Rust (via CGO FFI) | Compiled Stylus WASM modules.                            |
