...
:::info
PathDB snapshot auto-download with `--init.latest` is not yet supported.
:::
...
Use `--init.latest pruned` (or `archive`) and Nitro will handle multi-part downloads, checksum verification, and extraction automatically. Manual downloading is primarily useful when hosting a single snapshot locally for multiple nodes.
