...
- Snapshot can auto-download with:
  - `--init.latest` is not yet supported, full snapshot url needs to be specified in `--init.url` or the snapshot needs to be downloaded manually.
