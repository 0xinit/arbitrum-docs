...
- For running a path node, you can view the available [snapshots here]((https://snapshot-explorer.arbitrum.io/?chain=arb1).
...
If running a path node an additional flag is needed for starting the node: `--execution.caching.state-scheme=path`
