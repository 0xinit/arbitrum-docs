...
- Path based state schemes are supported for Nitro's state trie, but only for full nodes and archive nodes at this time and not validators yet. As a result, HashDB is still the default state scheme used by Nitro and explicit action is required to enable PathDB. More details can be found below in the table of Optional Parameters.
...
Latest [Docker image](https://hub.docker.com/r/offchainlabs/nitro-node/tags): <code>@@latestNitroNodeImage=offchainlabs/nitro-node:v3.9.7-75e084e@@</code>

### Database snapshots

::::info Snapshots availability

Database snapshots are available and located in the [snapshot explorer](https://snapshot-explorer.arbitrum.io/) for Arbitrum One, Arbitrum Nova, and Arbitrum Sepolia. At this time, only archive node snapshots using PathDB are available for Arbitrum One and Arbitrum Sepolia. Pruned PathDB snapshots for full nodes on Arbitrum One and Arbitrum Sepolia will be uploaded soon.

Database snapshots for other Arbitrum chains may be available at the discretion of the team running the chain. Please get in touch with them if you're interested in using a database snapshot for their chains.
...
- Only snapshots formatted with your node's specified state scheme will be compatible. In other words, a PathDB snapshot will not work for a node that is using HashDB and vice versa.
...
By default, all node types will use HashDB, go-ethereum's original state trie storage scheme. This will be the case until a path based state scheme (e.g. PathDB) is supported across all Arbitrum node types. This means that, by default, pruning will continue to be an *manual opt-in* operation, should an operator decide to do so. As a refresher, pruning a full node refers to the action of removing older, unnecessary data from the local copy of the blockchain that the node maintains, thereby saving disk space and slightly improving the node's efficiency. Pruning will remove all states from blocks older than the latest 128. 

For nodes using PathDB, pruning: will be automatic, can done so online, and default to retaining only the last 24hrs worth of state history on disk.

:::note
The pruning process occurs when the node start (upon initialization) and will not serve RPC requests during pruning.
:::
...
- `--init.prune <pruning mode>`, where `<pruning mode>` can be one of:
  ...
  - `validator`: Meant to be used by validator nodes and requires an RPC URL for L1 Ethereum. This mode retains the genesis state, the state of the latest confirmed block, latest confirmed assertion root (obtained from L1), the last locally validated block root,and the head state (at the latest snapshot). This mode is expected to take a little less than half what `full` pruning mode would take.
