...
Database snapshots for other Arbitrum chains may be available at the discretion of the chain's team. Please get in touch with them if you're interested in using a database snapshot for their chains.
...
- Only snapshots formatted with your node's specified state scheme will be compatible. In other words, a PathDB snapshot will not work for a node using HashDB, and vice versa.
