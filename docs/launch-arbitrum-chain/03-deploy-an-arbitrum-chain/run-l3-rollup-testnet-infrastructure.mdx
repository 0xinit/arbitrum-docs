...
After deploying sequencers, you **must** set at least one sequencer in the priority list. Without this, no sequencer will be chosen as active and your chain won't produce blocks. Complete **Step 10** to set priorities using the Sequencer Coordinator Manager, or set them directly in Redis.
...
You **must** set sequencer priority for your chain to work. Without setting priority, no sequencer will be chosen as active. The Sequencer Coordinator Manager (SQM) provides a UI to manage the sequencer priority list. You can also set priority directly in Redis (advanced users).

:::note Required for sequencer activation

This step is **required**, not optional. At least one sequencer must be added to the priority list in Redis for any sequencer to become active. The SQM provides the easiest way to do this.
...
1. **Select a sequencer** from the non-priority list using arrow keys and press Enter
