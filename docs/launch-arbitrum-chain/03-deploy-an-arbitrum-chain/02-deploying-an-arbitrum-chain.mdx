...
- Entering a contract requires reading its code from the database, which means more code = more work. The EVM doesn't charge differently based on code size, but the computation cost could vary; however, that doesn't seem to be a strict limit compared to the readability aspect.
...
**For L2's building on top of Ethereum (L1):**

The effective limit on calldata seems to be 128KB:

- Some clients (geth, AFAU) use 128KB as the limit, even though the protocol does not enforce it
...
**For L3's building on top of Arbitrum One:**

- The sequencer has a default limit on `TxMaxDataSize` of `95000`, so L3's should use a lower preimage limit than that, roughly 85KB.
