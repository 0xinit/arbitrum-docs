...
### "Mempool weight limit exceeded max mempool size"
...
- **Too many pending transactions**: The batch poster has submitted multiple transactions that have not yet been included in a block, and the parent chain node’s mempool has reached its limit for pending transactions from a single account or overall.
...
1. **Check the parent chain’s current gas prices** and compare them to your data poster’s fee cap configuration. If gas prices are above your max-fee-cap-gwei, the data poster cannot bid high enough to cover the fee. See [Batch poster fee tuning](/launch-arbitrum-chain/02-configure-your-chain/common/validation-and-security/batch-poster-fee-tuning.mdx) for guidance on adjustments.
2. **Review your queued transaction storage configuration**: If you are using DB or Redis storage, pending transactions accumulate and rely on RBF for inclusion. Consider whether your RBF schedule (blob-tx-replacement-times) is escalating fees quickly enough. See Queued transaction database selection for storage option details.
...
| Gas prices exceeding fee cap      | Parent chain gas prices are above `max-fee-cap-gwei`, causing the data poster to pause submissions                                                             |
...
2. **Check the parent chain gas prices**: if prices exceed your fee cap, either wait for prices to drop or increase `max-fee-cap-gwei` (see [Batch poster fee tuning](/launch-arbitrum-chain/02-configure-your-chain/common/validation-and-security/batch-poster-fee-tuning.mdx)).
3. **Verify parent chain connectivity**: ensures the batch poster node can reach the parent chain RPC endpoint and that the parent chain node is fully synced.
...
5. **Review node sync status**: confirms the batch poster node is synced with both the Arbitrum chain and the parent chain.
...
- **Storage inconsistencies**: When using DB or Redis for queued transaction tracking, mismatches between the stored transaction state and the onchain state can cause resubmission failures. This can happen after a node restart if the storage was not cleanly synced.
- **Parent chain rejection**: The parent chain node may reject the replacement transaction for various reasons—the replacement fee is not high enough (must be at least 10% higher for standard RBF), the transaction format is invalid, or the parent chain node is experiencing issues.

:::info Error context

The exact phrasing and context of retry errors can vary across Nitro versions. The “failed to re-send transaction” message generally indicates an RBF attempt that did not succeed. Check the surrounding log lines for more specific error details (e.g., “nonce too low”, “replacement transaction underpriced”, or connection errors).
...
| Batches not posting during gas spike    | Gas prices exceed `max-fee-cap-gwei`                  | Increase fee cap or wait for prices to normalize                    |
| "Failed to re-send transaction"         | Nonce conflict or RBF underpriced                     | Check if original tx was included; review fee escalation            |
