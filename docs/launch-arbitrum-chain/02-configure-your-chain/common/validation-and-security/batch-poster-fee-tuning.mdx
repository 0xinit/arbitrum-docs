...
- **Blob base fee**: Set by the protocol based on blob demand. This is not directly configurable—it is determined by the parent chain’s blob gas pricing mechanism.
...
When parent chain gas prices rise sharply, the data poster’s behavior is governed by fee cap parameters that control how high it will bid to get batch transactions included.

### Fee cap parameters

<!-- UNDOCUMENTED DEFAULTS

Unable to find this specific parameter in the dataposter.go, Found similar but unsure if the same parameters set to 1 or 5.

The max-fee-cap-gwei parameter sets the highest gas price the data poster can use for batch posting transactions. The specific default value is not included in the current documentation. Check the [Nitro source code](https://github.com/OffchainLabs/nitro/blob/master/arbnode/dataposter/dataposter.go) for the default or set it manually based on your cost limit.

-->

| Flag                                               | Description                                                                                                                                                                                                                              |
| -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--node.batch-poster.data-poster.max-fee-cap-gwei` | The maximum fee cap (in gwei) the data poster will use when posting transactions to the parent chain. If parent chain gas prices exceed this cap, the data poster will not post new transactions until prices fall below this threshold. |
...
When a batch transaction is pending in the parent chain mempool, the data poster uses RBF to gradually increase the gas price to improve inclusion chances. For blob transactions, this escalation schedule is controlled by:
...
Each duration triggers a replacement attempt with a higher fee. The schedule starts with an aggressive posture (five minutes, then ten minutes) and becomes more conservative over time. This prevents overspending while ensuring that transactions are eventually included.
...
3. If the transaction is not included, the RBF schedule triggers replacement attempts with incrementally higher fees.
4. If gas prices exceed `max-fee-cap-gwei`, the data poster pauses new batch submissions until prices decrease.
...
| Scenario                 | `max-blob-tx-tip-cap-gwei` | `min-blob-tx-tip-cap-gwei` | `max-fee-cap-gwei` | `blob-tx-replacement-times`                                  | Notes                                                                  |
| ------------------------ | -------------------------- | -------------------------- | ------------------ | ------------------------------------------------------------ | ---------------------------------------------------------------------- |
